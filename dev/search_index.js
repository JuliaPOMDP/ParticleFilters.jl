...

{
    ...
    "title": "Home",
    "category": "section",
    "text": "ParticleFilters.jl provides a basic particle filter representation along with some useful tools for constructing more complex particle filters.In particular it provides both weighted and unweighted particle belief types that implement the POMDPs.jl distribution interface including sampling and automatic caching of probability mass calculations.Additionally, an important requirement for a particle filter is efficient resampling. This package provides O(n) resamplers.Dynamics and measurement models for the filters can be specified as a ParticleFilterModel or a POMDP or a custom user-defined type.The simplest sampling-importance-resampling Particle filter can be constructed with SIRParticleFilter. BasicParticleFilter provides a more flexible structure.There are tutorials for three ways to use the particle filters:As an estimator for feedback control,\nto filter time-series measurements, and\nas an updater for POMDPs.jl.For documentation on all aspects of the package, see the contents below."
},
